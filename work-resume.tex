...
\noindent
\entrytitle{NACRE: Rethinking Confidential Containers through Native Architectural Support}
\hfill \daterange{2026.08 \textasciitilde{} 至今}

\par\noindent
$\bullet$ 负责将 QEMU 原型中的相关功能移植到 BOOM 处理器，并根据实现需求重构部分设计；基于 FireSim 完成 FPGA 上板部署。

\par\addvspace{0.35\baselineskip}
\noindent
\entrytitle{CPU Fuzzing ｜ RISC-V ISA 层错误检测}
\hfill \daterange{2025.09 \textasciitilde{} 至今}

\par\noindent
$\bullet$ 以 RISC-V 标准为依据开展 CPU fuzzing，寻找指令集架构（ISA）层面的行为错误。
...
\par\noindent
$\bullet$ \textbf{系统软件与验证：}开发处理器测试工具 Difftest，移植并成功启动 RT-Thread；为自己写的模拟器添加csr和页表支持并通过opensbi启动linux进入shell。
...
\noindent
\entrytitle{南京邮电大学[校/院]大学生科协电子部 ｜ 部长 / 讲师}
\hfill \daterange{2022.09 \textasciitilde{} 2023.06}

\par\noindent
$\bullet$ \textbf{组织与教学：}负责社团日常运营及面向大一学生的授课；策划校“科技节”中的比赛，保障赛程顺利推进。
